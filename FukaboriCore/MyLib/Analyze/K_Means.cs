...
{
    public class K_Means:System.ComponentModel.BackgroundWorker
    {
        private List<double[]> inputDataList = new List<double[]>();

        public List<double[]> InputDataList
        {
        ...
        public K_Means(int l):base()
        {
            length = l;
            this.DoWork += new System.ComponentModel.DoWorkEventHandler(K_Means_DoWork);
        ...
        void K_Means_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i < tryCount; i++)
			{
                ...
                this.ReportProgress((i+1) * 100 / tryCount);
            ...
            clusterDataList = list.OrderBy(n => n.Key).First().Value;
            e.Result = clusterDataList;
        }
        ...
        public class Cluster
        {
            ...
            public double GetClusterDistance()
            {
                ...
                foreach (double[] item in dataList)
                {
                    sum += MyLib.MathLib.GetDistance(clusterData, item);
                }
                if (dataList.Count > 0)
                {
        ...
        private double AnalyzeK_Means(int clusterCount, out List<Cluster> clusterList)
        {
            ...
            if (rType == RandamType.一つのデータ)
            {
                ...
                var shuffleList = inputDataList.OrderBy(n => Guid.NewGuid());
                ...
                foreach (var item in shuffleList)
                {
                    ...
                    {
                        if (item2.Data != null && item2.GetDistance(item) == 0)
                        {
                            flag = false;
                            break;
                        }
            ...
            if (rType == RandamType.複数の平均)
            {
                ...
                foreach (double[] item in inputDataList)
                {
            ...
            if (rType == RandamType.最小距離を使う)
            {
                ...
                {
                    List<KeyValuePair<double[], double>> tmpList = new List<KeyValuePair<double[], double>>();
                    for (int l = 0; l < inputDataList.Count; l++)
                    {
                        double min = double.MaxValue;
                        for (int k = 0; k < i; k++)
                        {
                            min = Math.Min(clusterList[k].GetDistance(inputDataList[l]), min);
                        }
                        if (min > 0)
                        {
                            tmpList.Add(new KeyValuePair<double[], double>(inputDataList[l], min));
                        }
                    }
                    var tmp2 = new List<double[]>(tmpList.OrderBy(n => n.Value).Select(n => n.Key));
                    if (tmp2.Count > 0)
                    {
            ...
            while (distance > 0)
            {
                ...
                foreach (double[] data in inputDataList)
                {
                    Cluster tmpCluster = SearchCluster(data, clusterList);
            ...
            double cc = inputDataList.Count;
            double sum = 0;
            double entropy = 1;
            foreach (Cluster item in clusterList)
            {
                sum += item.GetClusterDistance();
                entropy *= (double)item.DataCount / cc ;
            }

            return sum/Math.Log( entropy)*-1;
        ...
        public void Run(int clusterCount,int tryCount)
        {
