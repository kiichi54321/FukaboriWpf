...
namespace MyLib
{
    public class String
    {
        ...
        public static string[] TextToListByDelimiter(string html, string[] delimiter,string footer)
        {
            ...
            while (true)
            {
                endIndex = MyLib.String.IndexOfAny(html, delimiter, index, out useDelimiter);
                if (endIndex > -1)
                {
        ...
        /// <returns></returns>
        public static IEnumerable<string> GetTagContentList(string txt, string start, string end, bool tag)
        {
            int idx = 0;
            int s = 0;
            int e = 0;
            while (idx > -1)
            {
                ...
                if (tag)
                {
                    yield return txt.Substring(s, e - s + end.Length);
                }
                else
                {
                    yield return txt.Substring(s + start.Length, e - s - start.Length);
                }
                idx = e;
            }
        }
