...
namespace CrossTableSilverlight.Model
{
    ...
    public class Question : System.ComponentModel.INotifyPropertyChanged
    {
        ...
        public void SetImage(string t)
        {
            ...
            {
                ImageUrl = m.Value.Replace("{", "").Replace("}", "");
                text = t.Replace(m.Value, string.Empty);
            }
            OnPropertyChanged("Text");
        }
        ...
        public void SetType(string type)
        {
            if (type == "離散")
            {
                AnswerType = Model.AnswerType.離散;
                AnswerType2 = Model.AnswerType2.離散;
            }
            else if (type == "順序")
            {
                AnswerType = Model.AnswerType.順序;
                AnswerType2 = Model.AnswerType2.連続値;
            }
            else if (type == "数値")
            {
                AnswerType = Model.AnswerType.数値;
                AnswerType2 = Model.AnswerType2.連続値;
            }
            else if (type == "文字列")
            {
                AnswerType = Model.AnswerType.文字列;
                AnswerType2 = Model.AnswerType2.離散;
            }
            else if (type == "ラベル")
            {
                AnswerType = Model.AnswerType.ラベル;
                AnswerType2 = Model.AnswerType2.離散;
            }
            else
            {
                AnswerType = Model.AnswerType.不明;
                AnswerType2 = Model.AnswerType2.離散;
            }
        }
        ...
        {
            get { if (IsExtendQuestion) return Visibility.Visible;
        ...
        void Delete()
        {
            if (MessageBox.Show("本当に消しますか？","注意", MessageBoxButton.OKCancel) == MessageBoxResult.OK)
            {
        ...
        public QuestionAnswer GetOriginalValue(string value)
        {
            ...
            if (AnswerType == Model.AnswerType.数値)
            {
            ...
            else if (AnswerType == Model.AnswerType.文字列)
            {
                qa.TextValue = value;
                qa.Value = double.NaN;
                return qa;
            }
            else if(AnswerType == Model.AnswerType.不明)
            {
                qa.TextValue = value;
                qa.Value = double.NaN;
                return qa;
            ...
            if (Answers.Count > 0)
            {
                ...
                {
                    if (AnswerType != Model.AnswerType.ラベル)
                    {
                        qa.TextValue = "エラー";
                        qa.Value = double.NaN;
                        return qa;
        ...
        List<QuestionAnswer> GetOriginalValue()
        {
            ...
            int count = 1;
            if (this.AnswerType == Model.AnswerType.文字列 || this.AnswerType == Model.AnswerType.数値)
            {                
                return new List<QuestionAnswer>();
            
            }
            if(this.AnswerType == Model.AnswerType.ラベル)
            {
                foreach (var item in this.Answers)
                {
                    QuestionAnswer qa = new QuestionAnswer() { AnswerType = this.AnswerType };
        ...
        {
            if (answerGroupDic == null) answerGroupDic = new Dictionary<string, Model.AnswerGroup>();
