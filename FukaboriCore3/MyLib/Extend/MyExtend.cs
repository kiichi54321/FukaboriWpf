...
namespace RawlerLib.MyExtend
{
    ...
    public static class Char
    {
        ...
        {
            return s.ToCharArray().TakeWhile(n => n.Isひらがな() == false).JoinText("");
        }

        public static string TakeKatakana(this string s)
        {
            return s.ToCharArray().TakeWhile(n => n.Isカタカナ() == false).JoinText("");
        }

        public static string TakeAlphabets(this string s)
        {
            HashSet<char> alphas = new HashSet<char>("abcdefghijklmnopqrstuzwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".ToCharArray());
            return s.ToCharArray().TakeWhile(n => alphas.Contains(n)).JoinText("");
        }
    }
