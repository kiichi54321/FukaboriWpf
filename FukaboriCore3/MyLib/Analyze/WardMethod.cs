...
namespace MyLib.Analyze
{
    public class WardMethod 
    {
        ...
        private NormalizeType NormalizeType = NormalizeType.平均分散;
        ...
        private void Normalize()
        {
            ...
            if (NormalizeType == Analyze.NormalizeType.平均分散)
            {
                Dictionary<int, MyLib.Statistics.AvgStd> dic = new Dictionary<int, Statistics.AvgStd>();
                for (int i = 0; i < inputDataList.Min(n => n.Value.Length); i++)
                {
                    dic.Add(i, new Statistics.AvgStd());
                }
                foreach (var item in inputDataList)
                {
                    for (int i = 0; i < dic.Count; i++)
                    {
                        dic[i].Add(item.Value[i]);
                    }
                }
                foreach (var item in inputDataList)
                {
                    for (int i = 0; i < dic.Count; i++)
                    {
                        item.Value[i] = dic[i].標準化(item.Value[i]);
                    }                    
                }
            }
        ...
        void WardMethod_DoWork()
        {
            ...
            while (clusterList.Count > 1)
            {
                ...
                foreach (var n in pairList)
                {
                    n.Value = n.ClusterA.GetWardValue(n.ClusterB);
                }
                //MyLib.Task.Parallel.ForEach(pairList, (n) =>
                //{
                //    n.Value = n.ClusterA.GetWardValue(n.ClusterB);
                //});
        ...
        public Action<int> ReportProgress { get; set; } = (n) => { };
   ...
   //     System.Threading.Tasks.Task task = null;
        ...
        {
            await System.Threading.Tasks.Task.Factory.StartNew(()=> WardMethod_DoWork());
        }
