...
namespace MyWpfLib.Graph
{
	...
	public class Sort
	{
        ...
        public static Sort.Type ParseSortType(string str)
        {
            ...
            try
            {
                type = (Sort.Type)System.Enum.Parse(typeof(Sort.Type), str,false);
            }
            catch
            {
                type = Type.Jaccard;
        ...
        public static float 信頼度比(int match, int freq1, int freq2, int all)
        {
            float tmp1 = match / (float)freq1;
            float tmp2 = freq2 / (float)all;
            return (tmp1) / (tmp2);
        }

        public static float MutualInformation(int match, int freq1, int freq2, int all)
        {
            float p1_1 = match/(float)all;
            float p1_2 = (freq1 - match) / (float)all;
            float p2_1 = (freq2 - match)/(float)all;
            float p2_2 = (all - (freq1 + freq2 - match)) / (float)all;

            float px_a = p1_1 + p1_2;
            float px_b = p2_1 + p2_2;
            float py_a = p1_1 + p2_1;
            float py_b = p1_2 + p2_2;

            float hx = -1 * (float)(px_a * Math.Log(px_a, 2) + px_b * Math.Log(px_b, 2));
            float hy = -1 * (float)(py_a * Math.Log(py_a, 2) + py_b * Math.Log(py_b, 2));
            float hxy = -1 * (float)(p1_1 * Math.Log(p1_1, 2) + p1_2 * Math.Log(p1_2, 2) + p2_1 * Math.Log(p2_1, 2) + p2_2 * Math.Log(p2_2, 2));

            return hx + hy - hxy;
//            return (float)Math.Log((float)(freq1 * freq2) / (float)(match * all), 2);
        }
